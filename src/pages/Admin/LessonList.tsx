...
const LessonList: React.FC = () => {
  ...
  const handleEdit = (record: Lesson) => {
    const extendedRecord = record as ExtendedLesson;
    setEditingId(record.lessonId);
    setActiveTab(extendedRecord.lessonType || "normal");
    form.setFieldsValue(record);
    if (
      extendedRecord.lessonType === "quiz" &&
      extendedRecord.quiz &&
      Array.isArray(extendedRecord.quiz)
    ) {
      setQuizData({
        title: "",
        description: "",
        questions: extendedRecord.quiz,
      });
    }
    setShowForm(true);
  };
