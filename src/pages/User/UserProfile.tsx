...
const UserProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        try {
          const coursesRes = await fetch(`${API_URL}/users/${userId}/courses`, {
            method: "GET",
            headers: getAuthHeaders(),
          });
  ...
  const checkEnrollment = async (courseId: string) => {
    try {
      const res = await fetch(`${API_URL}/users/${userId}/enrollment/${courseId}`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
      const data = await res.json();
      const enrollment: Enrollment = data.data;
      ...
      <div className={styles.sidebar}>
        <Card className={styles.userCard}>
          ...
          <div className={styles.buttonGroup}>
            <Button type="primary" icon={<HomeOutlined />} block onClick={() => navigate("/")}>
              Trang chủ
            </Button>
          ...
          <List
            ...
            renderItem={(item) => (
              ...
              >
                ...
                <span>{item.text}</span>
                {item.badge !== undefined && <Badge count={item.badge} style={{ marginLeft: "auto" }} />}
              </List.Item>
            )}
          />
        </Card>
                  ...
                  renderItem={(course) => (
                    <List.Item>
                      <Card
                        ...
                        actions={[
                          <Button type="link" onClick={() => checkEnrollment(course.courseId)} key="check">
