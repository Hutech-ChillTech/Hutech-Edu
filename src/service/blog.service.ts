...
export const blogService = {
  ...
  getBlogPosts: async (params?: {
    ...
    const apiParams = {
      ...params,
      limit: params?.limit || (params as any)?.pageSize,
      category: params?.category || (params as any)?.categoryId,
      tag: params?.tag || (params as any)?.tagSlug,
    };
    const response = await axiosInstance.get(`${API_URL}/api/blog-posts`, { params: apiParams });
  ...
  getBlogPostById: async (blogPostId: string): Promise<BlogPost> => {
    const response = await axiosInstance.get(`${API_URL}/api/blog-posts/${blogPostId}`);
  ...
  getBlogPostBySlug: async (slug: string): Promise<BlogPost> => {
    const response = await axiosInstance.get(`${API_URL}/api/blog-posts/slug/${slug}`);
  ...
  createBlogPost: async (data: CreateBlogPostRequest): Promise<BlogPost> => {
    const response = await axiosInstance.post(`${API_URL}/api/blog-posts`, data);
  ...
  toggleBookmarkBlogPost: async (
    ...
    await axiosInstance.post(`${API_URL}/api/blog-posts/${blogPostId}/bookmark`, { note });
  },
  ...
  bookmarkBlogPost: async (blogPostId: string, note?: string): Promise<void> => {
    await axiosInstance.post(`${API_URL}/api/blog-posts/${blogPostId}/bookmark`, { note });
  },

  removeBookmark: async (blogPostId: string): Promise<void> => {
    // MD says toggle.
    await axiosInstance.post(`${API_URL}/api/blog-posts/${blogPostId}/bookmark`);
  },
  ...
  getBookmarkedPosts: async (): Promise<BlogPost[]> => {
    ...
    const response = await axiosInstance.get(`${API_URL}/api/blog-posts/bookmarks`);
    return response.data.data;
  },
  ...
  getFeaturedPosts: async (limit: number = 5): Promise<BlogPost[]> => {
    const response = await axiosInstance.get(`${API_URL}/api/blog-posts/featured`, {
      params: { limit }
    });
    return response.data.data;
  },
