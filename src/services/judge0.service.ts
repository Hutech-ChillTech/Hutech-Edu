...
import { ICodeSubmissionRequest, ICodeSubmissionResponse } from "../types/customRequest";
...
export const sendToJudge0 = async (data: ICodeSubmissionRequest) => {
  try {
    ...
    await codeSubmissionService.updateCodeSubmission(created.submissionId, submission.data);
    ...
    if (error.response?.data) {
      throw new Error(`Không thể gửi lên Judge0: ${JSON.stringify(error.response.data)}`);
    }
